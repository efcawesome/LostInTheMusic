...
public class MultipleMovingSpikeController : MonoBehaviour
{
    public List<GameObject> otherSpikes = new List<GameObject>();
    private IEnumerable<MovingSpike> otherSpikeControllers;
    ...
    void Start()
    {
        otherSpikeControllers = otherSpikes.Select(spike => spike.GetComponent<MovingSpike>());
    ...
    void Update()
    {
        ...
        if (!coroutineRunning && extendTimer <= 0)
        {
            ...
            foreach (MovingSpike otherSpike in otherSpikeControllers)
            {
                otherSpike.StartCoroutine(otherSpike.SwapState());
            }
        }
    }
